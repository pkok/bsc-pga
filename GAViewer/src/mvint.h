...
l3ga: projective 6D line geometric algebra for 3D Euclidean Geometry
...
#define MVI_IDEAL_PLANE (MVI_PLANE | MVI_IDEAL)
#define MVI_IDEAL_POINT (MVI_POINT | MVI_IDEAL)
...
public:
	...
	mvInt(const l3ga &mv, int creationFlags = 0);
	~mvInt();
	void initToNothing();
	...
	int interpret(const l3ga &mv, int creationFlags = 0);
