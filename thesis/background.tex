...
\begin{itemize}
  ...
  \item Left contraction $\lcont$ \comment{Is this still ``default'' notation, or should I use only $\dotp$/$\bullet$?}
...
while the following dependency relationship: 
\begin{equation} \label{eq:gaplucker0} 
  \ez \wedge (\V{q} - \V{p}) \wedge (\V{p} \wedge \V{q}) = 0 .
\end{equation}

Because of this constraint, lines only have 5 degrees of freedom.  As this expression is based on six coordinates, the last represents its weight, as defined in the homogeneous model.
...
The Pl\"ucker model uses these six elements as its basis, and treats them as atomic values.  These elements will be known as $\{\eze, \ezt, \ezd, \etd, \ede, \eet\}$.  Li and Zhang~\cite{Hongbo} define an embedding from the Pl\"ucker model to the homogeneous model:
\begin{equation*}
  \Em(x) = \left\{ 
    \begin{array}{ll}
