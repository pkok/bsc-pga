...
A special case are the ideal lines.  The homogeneous model treats these purely Euclidean elements as Euclidean 2-blades.\note{\LeosReply{waarom niet, hoe is dat anders dan het homogene model? Is e12 niet 'purely Euclidean omdat Em(e12) dat is? (ECHTE VRAAG)}

\myReply{Misschien moet ik wel stellen dat ideal lines twee interpretaties hebben; als Euclidean 2-blade, en als horizon?}}
...
%\end{table}
%
A 2-blade\note{\LeosReply{niet alle elementen van graad 2 zijn 2-blades. Dus hier moet iets bij.}

\myReply{Zou je een voorbeeld van een graad 2 niet-blade kunnen geven? Voor zover ik begrijp, zijn alle dingen met een graad blades, en anders multivectors van mixed grade.  Ik kan me geen graad-2-ding voorstellen dat niet een outer product of vectors is (def. van blade op p.195 van The Book).  Ik heb de \texttt{\\subsection}s veranderd van ``Elements of grade $k$'' naar ``Blades of grade $k$''; voldoende?}}
...
When $a$ and $b$ are coplanar\note{\LeosReply{or parallel}

\myReply{Per subsubsection 2.2.1: coplanar lines = (parallel lines) $\cup$ (intersecting lines)}}, their inner product is zero (see \autoref{eq:coplanar}), and each linear combination $c = \lambda a + \mu b$ is a null vector.  This kind of object is called a pencil of linear line complexes~\cite[Section 3.2.1]{Pottmann}, or, a bit shorter, a pencil of lines~\cite{Hongbo}.
...
Often one takes the point on the object closest to the chosen origin.  For a line $l = \plucker{\V{d}}{\V{m}}$, this is given by $p = \V{m} \times \V{d} + \V{d}^2 \ez$~\cite{Shoemake}.  Let $p_a$ and $p_b$ be such points for $a$ and $b$.  Define such a third point $p_c$ for line $c = p_a \wedge p_b$.  This is the point on the pencil closest to the origin. \note{\LeosReply{is dat zo? is dat echt zo? dat zou het punt zijn van het vlak waarin alle pencil-lijnen liggen. Is dat echt bepaald door de lijn die bepaald wordt door 2 toevallige dichtstbijzijnde punten van willekeurige representanten van die bundel? Misschien wel maar dat moet je dan even bewijzen.}

\myReply{Goed punt. Moet ik nog aan werken.}} 

\TODO{Prove/verify this!}

The dual of a pencil of lines contains the same set of lines~\cite[Section 3.2.1]{Pottmann}.  This can be easily shown.  Stating the definition of $\s{C}$ differently, we are looking for all lines that intersect with $C$.\note{\LeosReply{Wat bedoel je met intersect?
Als je bedoelt 'coincide'dan is dan de wedge-test.
Als je bedoelt: staan loodrecht op, dan is het de contraction test.}

\myReply{Dat weet ik nu niet eens zo zeker.  Het lijkt mij dat de contraction test hier ook ``snijdt elkaar in een (ideaal) punt'' betekent?}}  This is expressed as $c \wedge C = 0$.  For the grade-4 object $\dual{C}$, this can be written as $c \lcont \dual{C} = c \lcont (C \lcont \inverse{\ppseudo}) = (c \wedge C) \lcont \inverse{\ppseudo}$, with $\ppseudo$ the pseudoscalar of the space $\reals^{3,3}$.  This is only true for $c \wedge C = 0$, a test for containment of $c$ in $C$.
...
Now look at the case where $a$ and $b$ are not coplanar, but $a \wedge b$ still contains lines.  \autoref{eq:lincombnull} shows that either $\lambda = 0$ or $\mu = 0$, as $a \lcont b \not= 0$ per definition.  This means that all null vectors in $\s{C}$ must be homogeneously equivalent to $a$ or $b$.  This class of geometric objects is known as the hyperbolic pencil\note{\LeosReply{pencil
maar dit is toch gewoon een skew lijnenpaar? hyperbolic pencil is het 4-blade.}

\myReply{Over Fig. 3.6 zegt Pottmann dat het een hyperbolic linear congruence is, met focal lines $\mathcal A, B$.  Dat zou dus betekenen dat de hyperbolic pencil of linear complexes het skew lijnenpaar is.}} of linear complexes.  Its dual, the hyperbolic linear congruence, contains all lines intersecting both $a$ and $b$~\cite[Proposition 3.2.3]{Pottmann}. That is, for each point $p_a$ on $a$ and $p_b$ on $b$, the line $c$ with embedding $\Em(c) = p_a \wedge p_b$ is in $\dual{C}$.  
